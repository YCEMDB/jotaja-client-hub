...
BEGIN
  ...
  IF v_key NOT IN ('support_whatsapp','support_email','company_name','company_cnpj','public_url','maintenance_mode','maintenance_message','maintenance_bypass_emails') THEN
  ...
  IF v_key = 'maintenance_mode' THEN
    ...
    ELSE
      RAISE EXCEPTION 'invalid_setting_value' USING ERRCODE = '22023';
    END IF;
  ELSIF v_key IN ('maintenance_message','maintenance_bypass_emails') THEN
  ...
  ELSIF v_key = 'company_cnpj' THEN
    IF length(regexp_replace(v_text, '\D', '', 'g')) <> 14 THEN
      RAISE EXCEPTION 'invalid_setting_value' USING ERRCODE = '22023';
    END IF;
  ELSIF v_key = 'maintenance_bypass_emails' THEN
    IF length(v_text) > 4000 THEN
      RAISE EXCEPTION 'invalid_setting_value' USING ERRCODE = '22023';
    END IF;
    IF v_text <> '' AND EXISTS (
      SELECT 1
      FROM unnest(string_to_array(v_text, ',')) AS e(raw)
      WHERE btrim(e.raw) !~* '^[^@\s]+@[^@\s]+\.[^@\s]+$'
    ) THEN
      RAISE EXCEPTION 'invalid_setting_value' USING ERRCODE = '22023';
    END IF;
    v_new := to_jsonb(lower(v_text));
  END IF;
...
VALUES ('maintenance_bypass_emails', '""'::jsonb)
