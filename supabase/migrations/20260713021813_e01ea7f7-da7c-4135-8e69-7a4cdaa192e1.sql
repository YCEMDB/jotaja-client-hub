...
BEGIN
  ...
  IF NOT FOUND THEN
    UPDATE public.payment_webhook_events
     ...
     WHERE id = v_evt.id;
    RETURN jsonb_build_object('processed', true, 'applied', false, 'duplicate', false, 'error_code', 'payment_not_found');
  END IF;
  ...
  ELSIF v_ord.status = 'cancelled'::order_status THEN
    v_reject_code := 'order_cancelled';
  ...
  IF v_reject_code IS NOT NULL THEN
    ...
    UPDATE public.payment_webhook_events
     ...
     WHERE id = v_evt.id;
    RETURN jsonb_build_object(
      'processed', true, 'applied', false, 'duplicate', false,
      'error_code', v_reject_code, 'payment_id', v_pay.id, 'order_id', v_pay.order_id
    );
  END IF;
  ...
  IF p_new_status = 'paid' AND v_prev <> 'paid' THEN
    UPDATE public.orders SET payment_status = 'paid', paid_at = COALESCE(p_paid_at, now()) WHERE id = v_ord.id;
    IF v_ord.status = 'pending'::order_status THEN
  ...
  RETURN jsonb_build_object(
    ...
    'payment_id', v_pay.id, 'order_id', v_pay.order_id,
    'previous_status', v_prev, 'new_status', p_new_status, 'source', p_source
  );
END $function$;
