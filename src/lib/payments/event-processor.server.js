 ...
 * - Registrar resultado.
 */
export async function processPaymentEvent(webhookLogId, workerId) {
    ...
    try {
        ...
        const { data: log, error: fetchErr } = await supabaseAdmin
            ...
            .single();
        if (fetchErr || !log)
            throw new EventProcessorError("log_not_found", "Webhook log not found");
        if (!log.account)
            throw new EventProcessorError("account_not_found", "Associated payment account not found");
        // 3. Normalizar
        // Aqui importaríamos o normalizador para converter o payload
        const { PaymentNormalizer } = await import("./payment-normalizer.server");
        const internalEvent = PaymentNormalizer.normalize(log.provider, log.account_id, log.account.restaurant_id, log.payload);
        // 4. Registrar snapshot de processamento
        await supabaseAdmin.from("payment_processing_logs").insert({
            webhook_log_id: webhookLogId,
            status: 'PROCESSING',
            payload_snapshot: internalEvent,
...
async function validateStateTransition(restaurantId, event) {
