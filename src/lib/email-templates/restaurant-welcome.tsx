...
const RestaurantWelcomeEmail = ({
  ...
  <Html lang="pt-BR" dir="ltr">
    <Head />
    <Preview>
      {isReset ? `Sua senha do Mesivo foi redefinida` : `Bem-vindo ao Mesivo — ${restaurantName}`}
    </Preview>
    <Body style={main}>
      <Container style={container}>
        <Section style={brandBar}>
          <Text style={brandText}>MESIVO</Text>
        </Section>
        <Heading style={h1}>
        ...
        <Text style={text}>
          {isReset
            ...
            : `Seu restaurante ${restaurantName} acaba de ser criado no Mesivo. Abaixo estão suas credenciais de acesso ao painel. Recomendamos trocar a senha no primeiro login em "Configurações → Perfil".`}
        </Text>
...
export const template = {
  ...
  subject: (d: Record<string, any>) =>
    d?.isReset
      ? 'Sua senha do Mesivo foi redefinida'
      : `Bem-vindo ao Mesivo — ${d?.restaurantName ?? 'seu restaurante'}`,
