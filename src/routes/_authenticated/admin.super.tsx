...
import { LogIn, Search, ShieldCheck, AlertTriangle } from "lucide-react";
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Gestão de restaurantes, planos e assinaturas</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
                ...
                return (
                  <tr key={r.id} className="border-t hover:bg-muted/50">
                    ...
                    <td className="p-3 text-xs">
                      ...
                      <div className="text-muted-foreground">{r.owner_email ?? "—"}</div>
                    </td>
                    <td className="p-3">
                      <Badge variant="outline" className={PLAN_COLOR[r.plan]}>{PLAN_LABEL[r.plan]}</Badge>
                    </td>
                    <td className="p-3">
                      {r.is_active ? <Badge variant="outline" className="border-green-300 text-green-700">Ativo</Badge> : <Badge variant="outline" className="border-red-300 text-red-700">Inativo</Badge>}
                    </td>
                    <td className="p-3 text-xs">
                      <span className={trialExpired ? "text-destructive flex items-center gap-1" : ""}>
                        {trialExpired && <AlertTriangle className="h-3 w-3" />}
                    ...
                    <td className="p-3">
                      <div className="flex justify-end gap-1">
                        <Button size="sm" variant="ghost" title="Acessar painel" onClick={() => enterRestaurant(r)}>
              ...
              {!filtered.length && (
                <tr><td colSpan={9} className="p-8 text-center text-muted-foreground">Nenhum restaurante encontrado</td></tr>
              )}
            </tbody>
          </table>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between rounded border p-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Inativos não aparecem no marketplace público</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input type="date" value={editing.trial_ends_at?.slice(0, 10) ?? ""}
                      onChange={(e) => setEditing({ ...editing, trial_ends_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
                  </div>
                  <div>
                    <Label className="text-xs">Fim da assinatura</Label>
                    <Input type="date" value={editing.subscription_ends_at?.slice(0, 10) ?? ""}
                      onChange={(e) => setEditing({ ...editing, subscription_ends_at: e.target.value ? new Date(e.target.value).toISOString() : null })} />
                  </div>
                </div>
